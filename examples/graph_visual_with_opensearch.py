"""
Knowledge Graph Visualization with OpenSearch + madRAG WebUI

This script demonstrates two ways to visualize the knowledge graph
stored in OpenSearch:

1. **WebUI (recommended)**: Opens the madRAG WebUI in your browser
   for interactive graph exploration with search, filtering, and
   force-directed layout.

2. **Standalone HTML**: Fetches graph data from the madRAG Server API
...
Prerequisites:
    1. madRAG Server running with OpenSearch storage:
       madrag-server --host 0.0.0.0 --port 9621
...
def fetch_graph(server_url: str, label: str = "*", max_nodes: int = 300) -> dict:
    """Fetch knowledge graph data from madRAG Server API."""
...
def main():
    parser = argparse.ArgumentParser(
        description="Visualize madRAG knowledge graph from OpenSearch"
    )
    parser.add_argument(
        "--html",
        action="store_true",
    ...
    parser.add_argument(
        ...
        help="madRAG Server URL (default: http://localhost:9621)",
    )
    parser.add_argument(
        "--output",
    ...
    try:
        ...
        print(f"Error: Cannot connect to madRAG Server at {args.server}")
        print("Start the server first: madrag-server --host 0.0.0.0 --port 9621")
        sys.exit(1)
    ...
    if args.html:
        ...
        print(f"Opening madRAG WebUI graph explorer: {url}")
        webbrowser.open(url)

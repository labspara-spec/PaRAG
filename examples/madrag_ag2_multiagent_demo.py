"""madRAG + AG2 Multi-Agent Demo.

Demonstrates how AG2 agents can use madRAG's knowledge graph retrieval
as a tool. Multiple specialized agents collaborate to answer complex
questions over indexed documents.

Architecture:
    User -> AG2 GroupChat (Researcher + Analyst + Writer) -> madRAG queries
    - Researcher: uses madRAG hybrid search to gather facts
    - Analyst: uses madRAG naive (vector) search for complementary results
    - Writer: synthesizes findings into a final answer

Requires:
    pip install madrag-hku "ag2[openai]>=0.11.4,<1.0"
    export OPENAI_API_KEY="..."

Usage:
    python examples/madrag_ag2_multiagent_demo.py
"""
...
madRAG implements this approach with dual-level retrieval: local search focuses
...
async def setup_madrag() -> madRAG:
    """Initialize madRAG and index sample documents."""
    ...
    print("madRAG initialized and documents indexed.\n")
    return rag


# --- Async Bridge ---
# AG2 runs tools in a background thread without an event loop.
# We maintain a dedicated event loop in a separate thread for madRAG async calls.
...
# Global reference to madRAG instance (set in main)
_rag_instance: madRAG = None


def create_agents():
    """Create AG2 agents with madRAG tools."""
    ...
    researcher = AssistantAgent(
        name="Researcher",
        system_message=(
            "You are a research specialist. Use the madrag_query tool to search "
    ...
    analyst = AssistantAgent(
        ...
        system_message=(
            ...
            "the madrag_query tool with mode='naive' to run a direct vector search. "
    ...
    # --- Register madRAG as a tool ---

    @user_proxy.register_for_execution()
    @researcher.register_for_llm(
        description=(
            "Query the madRAG knowledge base. "
    ...
    def madrag_query(query: str, mode: str = "hybrid") -> str:
        """Query madRAG synchronously (wraps async call)."""
...
def run_multiagent_query(user_proxy, researcher, analyst, writer, question: str):
    """Run a multi-agent GroupChat to answer a question using madRAG."""
...
def main():
    ...
    # Start a background event loop for madRAG async calls.
    ...
    try:
        # Step 1: Set up madRAG (async, runs on the background loop)
        _rag_instance = _run_async(setup_madrag())

        # Step 2: Create AG2 agents with madRAG tools

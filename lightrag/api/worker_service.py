"""
lightrag-worker — standalone document-processing worker service.

Run one or more of these alongside the API servers in cloud deployments:

    lightrag-worker --workers 4

Each worker process:
1. Connects to Redis and subscribes to the lightrag:jobs stream.
2. Maintains a local LRU cache of LightRAG instances (one per workspace).
...
logger = logging.getLogger("lightrag.worker")
...
async def _get_cached_instance(
    ...
    """Return cached LightRAG for workspace, or create a new one."""
...
async def _run_worker(worker_id: str, args: argparse.Namespace) -> None:
    ...
    from lightrag.api.sse_bus import RedisSSEBus, RedisPublishBus
    ...
    # Build a LightRAG factory using the same config as the API server would.
    # We import the factory builder from lightrag_server to avoid duplicating
    # configuration logic.
    from lightrag.api.lightrag_server import build_rag_factory
...
def _build_parser() -> argparse.ArgumentParser:
    ...
    p = argparse.ArgumentParser(
        prog="lightrag-worker",
        description="PaRAG document-processing worker for cloud deployments",
    )
    p.add_argument(
        "--workers",
        type=int,
    ...
    p.add_argument(
        ...
        help="Max LightRAG instances this worker caches locally",
    )
    p.add_argument(
        "--worker-idle-timeout",
        type=float,
    ...
    # LightRAG instance with the same configuration as the API server.

...
One LightRAG instance is kept alive per workspace.  Instances are created on
...
class PooledInstance:
    rag: object  # LightRAG — typed as object to avoid circular import
    workspace: str
    created_at: float
    last_used_at: float
    ref_count: int = 0
...
class LightRAGPool:
    """
    Workspace-keyed pool of LightRAG instances with TTL-based eviction.

    Parameters
    ----------
    factory:
        Async callable ``factory(workspace: str) -> LightRAG`` that creates
    ...
    async def acquire(self, workspace: str) -> AsyncGenerator[object, None]:
        """
        Context manager that returns the LightRAG instance for *workspace*.
    ...
    async def initialize(self, default_workspace: str) -> None:
        ...
        logger.info(
            "LightRAGPool started: max_size=%d min_size=%d idle_timeout=%.0fs",
    ...
    async def shutdown(self) -> None:
        ...
        logger.info("LightRAGPool shut down; %d instances finalized.", len(workspaces))
    ...
    async def _create_instance_locked(self, workspace: str) -> PooledInstance:
        logger.info("LightRAGPool: creating instance for workspace=%s", workspace)
    ...
    async def _evict_lru_locked(self) -> None:
        ...
        if not idle:
            raise RuntimeError(
                "LightRAGPool exhausted: all instances are in use "
                f"(max_size={self._max_size}). "
                "Increase RAG_POOL_MAX_SIZE or reduce concurrent workspaces."
            )
        ws, _ = min(idle, key=lambda x: x[1].last_used_at)
        logger.info("LightRAGPool: LRU-evicting workspace=%s to make room", ws)
    ...
    async def _finalize_rag(rag: object, workspace: str) -> None:
        try:
            ...
            logger.info("LightRAGPool: finalized workspace=%s", workspace)
        except Exception as exc:
            logger.warning(
                "LightRAGPool: error finalizing workspace=%s: %s", workspace, exc
            )
    ...
    async def _cleanup_loop(self) -> None:
        while True:
            try:
                ...
                logger.warning("LightRAGPool cleanup error: %s", exc)
    ...
    async def _cleanup_stale(self) -> None:
        ...
        for ws in to_evict:
            ...
            logger.info(
                "LightRAGPool: evicted idle workspace=%s (idle=%.0fs)",
                ws,

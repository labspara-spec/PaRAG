...
def get_cloud_provider(args=None) -> Optional[CloudStorageProvider]:
    ...
    if provider_name == "s3":
        from lightrag.storage.cloud.s3 import S3StorageProvider
        return S3StorageProvider.from_env()

    if provider_name == "azure_blob":
        from lightrag.storage.cloud.azure_blob import AzureBlobStorageProvider
        return AzureBlobStorageProvider.from_env()

    if provider_name in ("sharepoint", "onedrive"):
        from lightrag.storage.cloud.graph import GraphStorageProvider
        return GraphStorageProvider.from_env()

    if provider_name == "google_drive":
        from lightrag.storage.cloud.google_drive import GoogleDriveStorageProvider
        return GoogleDriveStorageProvider.from_env()

    if provider_name == "gcs":
        from lightrag.storage.cloud.gcs import GCSStorageProvider

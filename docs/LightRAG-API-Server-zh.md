...
当前支持视觉输入的 provider 包括 `openai`、`azure_openai`、`gemini`、`bedrock` 和 `anthropic`；`lollms` 不能用于 VLM。典型配置：
...
| `--llm-binding` | `openai` | LLM 绑定类型（`lollms`、`openai`、`azure_openai`、`bedrock`、`gemini`） |
| `--embedding-binding` | `openai` | Embedding 绑定类型（`lollms`、`openai`、`azure_openai`、`bedrock`、`jina`、`gemini`、`voyageai`） |

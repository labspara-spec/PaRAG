# madRAG Sidecar 文件格式说明

本文介绍内解析引擎输出的**madRAG Sidecar**文件格式。madRAG 在使用native/mineru/docling这些支持多模态内容解析引擎提取文件内容的时候，会把"正文 + 多模态对象 + 解析元数据"拆开写到一个 `*.parsed/` 目录中，目录内的每个 JSON / JSONL 文件统称为 **sidecar** 文件。Sidecar 是后续流水线（多模态分析 → 多模态 chunk 构造 → 实体抽取 → 文档删除时的缓存清理）唯一可靠的依据。Sidecar的文件格式是madRAG内置的通用文件交换格式，新的多模态内容提取引擎都需要遵循这个格式。公开**madRAG Sidecar**文件格式的目的是给社区开发者编写字节的内容解析引擎提供方便。
...
- 多模态分析阶段 (`analyze_multimodal`) 由 madRAG 写入分析结果 `llm_analyze_result` 字典，可能是首次追加，也可能覆盖已有结果；解析器不应预先填充该字段
...
> madRAG要求同一个workspace（知识库）内的文件名（document_name）必须唯一。
...
`surrounding.leading` 和 `surrounding.trailing` 是 sidecar item 的可分析上下文窗口，目的是提供图片、表格和公式所在段落的上下文信息，提高多模态分析的质量。**surrounding内容有madRAG在分析阶段自动注入，不需要在文档解析引擎中主动写入sidecar文件中**。以下是surrounding内容的生成逻辑：
...
图形 `type` 受 12 项枚举约束（见 [`IMAGE_TYPE_ENUM`](../madrag/prompt_multimodal.py)：`Photo / Illustration / Screenshot / Icon / Chart / Table / Infographic / Flowchart / Chat Log / Wireframe / Texture / Other`）；模型若返回枚举外的值，会被规整成 `Other` 而不是失败。

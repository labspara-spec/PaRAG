...
This tool is used to locally debug madRAG's three content parsing engines (`native` / `mineru` / `docling`). It triggers the `madRAG.parse_<engine>` production code path for a **single file** and outputs the parsing artifacts (sidecar and raw cache) into a **flat directory layout**. Compared with the production ingestion directory, the only differences are:
...
```bash
python -m madrag.parser.cli <input_file> \
...
```bash
python -m madrag.parser.cli ./inputs/workspace/sample.docx --engine native
...
python -m madrag.parser.cli ./inputs/workspace/sample.pdf --engine mineru
...
```bash
python -m madrag.parser.cli ./inputs/workspace/sample.pdf \
...
## Equivalence with the `madRAG.parse_*` Production Path

This CLI directly calls the production code paths `madRAG.parse_native` / `parse_mineru` / `parse_docling` (via the lightweight RAG stand-in in `madrag/parser/debug.py`), so:

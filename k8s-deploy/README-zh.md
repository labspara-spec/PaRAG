...
这是用于在Kubernetes集群上部署madRAG服务的Helm chart。
...
您可以使用提供的便捷脚本或直接使用Helm命令部署madRAG。两种方法都配置了`madrag/values.yaml`文件中定义的相同环境变量。
...
madRAG及其数据库部署在同一Kubernetes集群中，使配置变得简单。
...
您可以通过修改`values.yaml`文件来配置madRAG的资源使用：
...
`values.yaml`文件中的`env`部分包含madRAG的所有环境配置，类似于`.env`文件。当使用helm upgrade或helm install命令时，可以使用--set标志覆盖这些变量。

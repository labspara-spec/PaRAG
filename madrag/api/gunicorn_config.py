...
from madrag.utils import setup_logger, get_env_value
...
errorlog = os.getenv("ERROR_LOG", log_file_path)  # Default write to madrag.log
accesslog = os.getenv("ACCESS_LOG", log_file_path)  # Default write to madrag.log
...
def on_starting(server):
    ...
    # Log the location of the madRAG log file
    print(f"madRAG log file: {log_file_path}\n")
...
def post_fork(server, worker):
    ...
    setup_logger("madrag", log_level, add_filter=True, log_file_path=log_file_path)

    # Set up madrag submodule loggers
    for name in logging.root.manager.loggerDict:
        if name.startswith("madrag."):

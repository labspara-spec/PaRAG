...
class ClientManager:
    """Process-wide singleton for QdrantClient, shared across all madRAG instances.

    Mirrors the ClientManager pattern used by postgres_impl, redis_impl, mongo_impl,
    and opensearch_impl so that creating multiple madRAG instances (e.g. one per
...
def _find_legacy_collection(
    ...
    created by older versions of madRAG:

    1. madrag_vdb_{namespace} - if model_suffix is provided (HIGHEST PRIORITY)
    2. {workspace}_{namespace} or {namespace} - no matter if model_suffix is provided or not
    3. madrag_vdb_{namespace} - fall back value no matter if model_suffix is provided or not (LOWEST PRIORITY)
    ...
    candidates = [
        f"madrag_vdb_{namespace}" if model_suffix else None,
        f"{workspace}_{namespace}" if workspace else None,
        f"madrag_vdb_{namespace}",
        namespace,
    ]

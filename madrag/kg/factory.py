...
from madrag.kg import STORAGES
...
def get_storage_class(storage_name: str) -> Callable[..., Any]:
    ...
    if storage_name == "NanoVectorDBStorage":
        from madrag.kg.nano_vector_db_impl import NanoVectorDBStorage
    ...
    if storage_name == "JsonDocStatusStorage":
        from madrag.kg.json_doc_status_impl import JsonDocStatusStorage
    ...
    # against the top-level ``madrag`` package, so anchor the import there
    # rather than letting it resolve against this module's own package.
    import_path = STORAGES[storage_name]
    module = importlib.import_module(import_path, package="madrag")

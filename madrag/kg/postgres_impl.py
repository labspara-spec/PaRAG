...
from madrag.types import KnowledgeGraph, KnowledgeGraphNode, KnowledgeGraphEdge
...
class PostgreSQLDB:
    ...
    async def _migrate_llm_cache_schema(self):
        ...
        try:
            ...
            WHERE table_name = 'madrag_llm_cache'
    ...
    async def _migrate_doc_chunks_to_vdb_chunks(self):
        ...
        try:
            ...
            WHERE table_name = 'madrag_doc_chunks' AND column_name = 'content_vector'
            """
    ...
    async def _migrate_doc_status_add_chunks_list(self):
        ...
        try:
            ...
            SELECT column_name
            FROM information_schema.columns
            WHERE table_name = 'madrag_doc_status'
    ...
    async def _migrate_text_chunks_add_llm_cache_list(self):
        ...
        try:
            ...
            SELECT column_name
            FROM information_schema.columns
            WHERE table_name = 'madrag_doc_chunks'
    ...
    async def _migrate_doc_status_add_track_id(self):
        ...
        try:
            ...
            SELECT indexname
            FROM pg_indexes
            WHERE tablename = 'madrag_doc_status'
            AND indexname = 'idx_madrag_doc_status_track_id'
            """
            ...
            if not index_info:
                ...
                create_index_sql = """
                CREATE INDEX idx_madrag_doc_status_track_id ON LIGHTRAG_DOC_STATUS (track_id)
                """
    ...
    async def _migrate_doc_full_add_pipeline_fields(self):
        ...
        try:
            existing = await self.query(
                ...
                SELECT column_name
                FROM information_schema.columns
                WHERE table_name = 'madrag_doc_full'
    ...
    async def _migrate_doc_status_add_content_hash(self):
        ...
        try:
            ...
            WHERE tablename = 'madrag_doc_status'
              AND indexname = 'idx_madrag_doc_status_workspace_content_hash'
            """
            index_info = await self.query(check_index_sql)
            if not index_info:
                logger.info(
                    "Creating partial index idx_madrag_doc_status_workspace_content_hash"
                )
                await self.execute(
                    """
                    CREATE INDEX IF NOT EXISTS idx_madrag_doc_status_workspace_content_hash
    ...
    async def _create_pagination_indexes(self):
        ...
        indexes = [
            {
                "name": "idx_madrag_doc_status_workspace_status_updated_at",
                "sql": "CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_madrag_doc_status_workspace_status_updated_at ON LIGHTRAG_DOC_STATUS (workspace, status, updated_at DESC)",
                "description": "Composite index for workspace + status + updated_at pagination",
            },
            {
                "name": "idx_madrag_doc_status_workspace_status_created_at",
                "sql": "CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_madrag_doc_status_workspace_status_created_at ON LIGHTRAG_DOC_STATUS (workspace, status, created_at DESC)",
                "description": "Composite index for workspace + status + created_at pagination",
            },
            {
                "name": "idx_madrag_doc_status_workspace_updated_at",
                "sql": "CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_madrag_doc_status_workspace_updated_at ON LIGHTRAG_DOC_STATUS (workspace, updated_at DESC)",
                "description": "Index for workspace + updated_at pagination (all statuses)",
            },
            {
                "name": "idx_madrag_doc_status_workspace_created_at",
                "sql": "CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_madrag_doc_status_workspace_created_at ON LIGHTRAG_DOC_STATUS (workspace, created_at DESC)",
                "description": "Index for workspace + created_at pagination (all statuses)",
            },
            {
                "name": "idx_madrag_doc_status_workspace_id",
                "sql": "CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_madrag_doc_status_workspace_id ON LIGHTRAG_DOC_STATUS (workspace, id)",
                "description": "Index for workspace + id sorting",
            },
            {
                "name": "idx_madrag_doc_status_workspace_file_path",
                "sql": "CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_madrag_doc_status_workspace_file_path ON LIGHTRAG_DOC_STATUS (workspace, file_path)",
...
class ClientManager:
    ...
    def _assert_compatible_vector_signature(
        ...
        raise RuntimeError(
            ...
            f"{requested_signature}. Multiple madRAG instances with different "

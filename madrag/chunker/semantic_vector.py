...
The chunker exposed here is ``async`` because madRAG's
...
Caveats:
  - SemanticChunker does NOT enforce a maximum chunk size; the caller's
    ...
    :func:`madrag.utils.enforce_chunk_token_limit_before_embedding`.
  - When ``embedding_func`` is ``None`` we log a warning and fall back to
    :func:`madrag.chunker.chunking_by_recursive_character` — V's only
...
from madrag.constants import DEFAULT_SENTENCE_SPLIT_REGEX
from madrag.utils import EmbeddingFunc, Tokenizer, logger
...
class _AsyncEmbeddingFuncAdapter(Embeddings):
    """Bridge a madRAG :class:`EmbeddingFunc` (async) to LangChain's
...
async def chunking_by_semantic_vector(
    ...
    *,
    ...
    Args:
        tokenizer: madRAG tokenizer (used for output token counts).
        ...
        embedding_func: madRAG :class:`EmbeddingFunc`. When ``None``

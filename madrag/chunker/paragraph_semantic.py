"""Paragraph Semantic Chunking for madRAG.

Reads a madRAG ``.blocks.jsonl`` artifact (produced by the docx native
parser at ``fixlevel=0`` — heading-driven splits only, tables kept whole)
and produces a chunk list compatible with
:func:`madrag.chunker.chunking_by_token_size`.
...
``madrag/parser/docx/parse_document.py``.
...
from madrag.table_markup import (
...
from madrag.utils import Tokenizer, logger


# ---------------------------------------------------------------------------
# Threshold ratios — derived from the audit-mode constants in
# madrag/parser/docx/parse_document.py so the trade-off curves
...
# imported from :mod:`madrag.table_markup` so the surrounding-context
...
def _split_long_block(
    ...
    *,
    ...
    Mirrors :func:`madrag.parser.docx.parse_document.split_long_block`,
...
def _merge_small_blocks(
    ...
    *,
    ...
    :func:`madrag.parser.docx.parse_document.merge_small_blocks`,
...
def chunking_by_paragraph_semantic(
    ...
    *,
    ...
    Signature follows the madRAG chunker contract — the standard
    ...
    Args:
        tokenizer: madRAG tokenizer (used for all token counting; matches

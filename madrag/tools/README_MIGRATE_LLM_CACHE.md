...
This tool migrates madRAG's LLM response cache between different KV storage implementations. It specifically migrates caches generated during file extraction (mode `default`), including entity extraction and summary caches.
...
The LLM Cache Migration Tool reads the storage configuration of the madRAG Server and provides an LLM migration option to select source and destination storage. Ensure that both the source and destination storage have been correctly configured and are accessible via the madRAG Server before cache migration.

## Usage

### Basic Usage

Run from the madRAG project root directory:

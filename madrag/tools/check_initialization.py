#!/usr/bin/env python3
"""
Diagnostic tool to check madRAG initialization status.

This tool helps developers verify that their madRAG instance is properly
...
Usage:
    ...
    await check_madrag_setup(rag, verbose=True)

    # Run demo from command line:
    python -m madrag.tools.check_initialization --demo
"""
...
from madrag.base import StoragesStatus


async def check_madrag_setup(rag_instance: madRAG, verbose: bool = False) -> bool:
    """
    Check if a madRAG instance is properly initialized.

    Args:
        rag_instance: The madRAG instance to check
    ...
    print("🔍 Checking madRAG initialization status...\n")

    # Check storage initialization status
    if not hasattr(rag_instance, "_storages_status"):
        issues.append("madRAG instance missing _storages_status attribute")
    ...
    if issues:
        ...
        print(
            "\n📚 Documentation: https://github.com/HKUDS/madRAG#important-initialization-requirements"
        )
        ...
        print("✅ madRAG is properly initialized and ready to use!")
...
async def demo():
    ...
    from madrag.llm.openai import openai_embed, gpt_4o_mini_complete

    print("=" * 50)
    print("madRAG Initialization Diagnostic Tool")
    print("=" * 50)
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Check madRAG initialization status")
    parser.add_argument(
    ...
    if args.demo:
        ...
        print("Or import this module and use check_madrag_setup() with your instance")

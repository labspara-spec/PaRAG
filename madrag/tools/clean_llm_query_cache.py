#!/usr/bin/env python3
"""
LLM Query Cache Cleanup Tool for madRAG
...
Usage:
    python -m madrag.tools.clean_llm_query_cache
    # or
    python madrag/tools/clean_llm_query_cache.py
...
from madrag.kg import STORAGE_ENV_REQUIREMENTS
from madrag.kg.shared_storage import set_all_update_flags
...
class CleanupTool:
    ...
    def get_storage_class(self, storage_name: str):
        ...
        if storage_name == "JsonKVStorage":
            ...
            from madrag.kg.redis_impl import RedisKVStorage

            return RedisKVStorage
        elif storage_name == "PGKVStorage":
            from madrag.kg.postgres_impl import PGKVStorage

            return PGKVStorage
        elif storage_name == "MongoKVStorage":
            from madrag.kg.mongo_impl import MongoKVStorage

            return MongoKVStorage
        elif storage_name == "OpenSearchKVStorage":
            from madrag.kg.opensearch_impl import OpenSearchKVStorage
    ...
    async def count_query_caches_pg(self, storage) -> Dict[str, Dict[str, int]]:
        ...
        Returns:
            Dictionary with counts for each mode and cache_type
        """
        from madrag.kg.postgres_impl import namespace_to_table_name
    ...
    def print_header(self):
        """Print tool header"""
        print("\n" + "=" * 60)
        print("LLM Query Cache Cleanup Tool - madRAG")
        print("=" * 60)
    ...
    async def setup_storage(self) -> tuple:
        ...
        if storage_name == "JsonKVStorage":
            ...
            print("  • madRAG Server is completely shut down")
            print("  • No other programs are accessing the storage files")
            print("\n" + "=" * 60)

            confirm = (
                input("\nHas madRAG Server been shut down? (yes/no): ")
                .strip()
                .lower()
            )
            if confirm != "yes":
                print(
                    "\n✓ Operation cancelled - Please shut down madRAG Server first"
                )
                return None, None, None
    ...
    async def run(self):
        ...
        try:
            ...
            try:
                from madrag.kg.shared_storage import finalize_share_data

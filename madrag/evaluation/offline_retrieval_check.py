#!/usr/bin/env python3
"""Offline sanity check for the bundled madRAG evaluation samples.

The check uses a small deterministic lexical ranker. It does not start
madRAG, call the API server, compute embeddings, or call LLM/RAGAS services.
"""
...
def print_report(results: list[QueryResult], top_k: int) -> None:
    ...
    print("madRAG sample retrieval check")
...
def parse_args(argv: list[str]) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Run an offline retrieval check for madRAG evaluation samples."
    )

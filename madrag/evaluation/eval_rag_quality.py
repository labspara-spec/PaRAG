#!/usr/bin/env python3
"""
RAGAS Evaluation Script for madRAG System
...
Usage:
    ...
    python madrag/evaluation/eval_rag_quality.py -d my_test.json

    # Specify custom RAG endpoint
    python madrag/evaluation/eval_rag_quality.py --ragendpoint http://my-server.com:9621
    python madrag/evaluation/eval_rag_quality.py -r http://my-server.com:9621
...
Results are saved to: madrag/evaluation/results/
...
from madrag.utils import logger
...
class RAGEvaluator:
    ...
    def __init__(self, test_dataset_path: str = None, rag_api_url: str = None):
        ...
        Args:
            ...
            rag_api_url: Base URL of madRAG API (e.g., http://localhost:9621)
    ...
    def _display_configuration(self):
        ...
        logger.info("  • madRAG API:         %s", self.rag_api_url)
    ...
    async def generate_rag_response(
        ...
        """
        Generate RAG response by calling madRAG API.
        ...
        Raises:
            Exception: If madRAG API is unavailable.
        """
        try:
            payload = {
                "query": question,
            ...
            raise Exception(
                f"❌ Cannot connect to madRAG API at {self.rag_api_url}\n"
                f"   Make sure madRAG server is running:\n"
                f"   python -m madrag.api.madrag_server\n"
                f"   Error: {str(e)}"
            )
        except httpx.HTTPStatusError as e:
            raise Exception(
                f"madRAG API error {e.response.status_code}: {e.response.text}"
            )
            ...
            raise Exception(f"Error calling madRAG API: {type(e).__name__}: {str(e)}")
    ...
    async def evaluate_responses(self) -> List[Dict[str, Any]]:
        ...
        logger.info("%s", "=" * 70)
        logger.info("🚀 Starting RAGAS Evaluation of madRAG System")
...
async def main():
    ...
    Command-line arguments:
        ...
        --ragendpoint, -r: madRAG API endpoint URL (default: http://localhost:9621 or $LIGHTRAG_API_URL)
    ...
    try:
        ...
        parser = argparse.ArgumentParser(
            description="RAGAS Evaluation Script for madRAG System",
...
Examples:
  ...
  python madrag/evaluation/eval_rag_quality.py -d my_test.json -r http://localhost:9621
        ...
        parser.add_argument(
            ...
            type=str,
            default=None,
            help="madRAG API endpoint URL (default: http://localhost:9621 or $LIGHTRAG_API_URL environment variable)",
        )

        args = parser.parse_args()

        logger.info("%s", "=" * 70)
        logger.info("🔍 RAGAS Evaluation - Using Real madRAG API")
        logger.info("%s", "=" * 70)

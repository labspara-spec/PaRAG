...
These helpers are shared by the madRAG pipeline mixin (madrag/pipeline.py)
and by other madRAG methods that touch the document ingestion paths
...
from madrag.base import DocProcessingStatus, DocStatus, DocStatusStorage
from madrag.chunk_schema import CHUNK_PERMISSION_FIELDS, normalize_chunk_section_meta
from madrag.constants import (
    FULL_DOCS_FORMAT_LIGHTRAG,
    LIGHTRAG_DOC_CONTENT_PREFIX,
    PARSED_DIR_NAME,
)
from madrag.parser.routing import canonicalize_parser_hinted_basename
...
def compute_file_content_hash(path_str: str) -> str | None:
    ...
    Resolves the madRAG ``*.blocks.jsonl`` conventions used by
    ``_load_madrag_document_content`` so the hash matches the actual
...
def make_madrag_doc_content(merged_text: str) -> str:
    """Build the ``full_docs.content`` value for ``format=madrag`` records.

    The result has shape ``"{{LRdoc}}<merged_text>"`` — the marker prefix
    distinguishes madrag-format full_docs from raw-format ones, and the
    body is the complete merged text from the ``.blocks.jsonl`` content
    lines so F-chunking can run identically on raw and madrag inputs
...
def strip_madrag_doc_prefix(content: str | None, parse_format: str | None) -> str:
    """Return the bare body for a stored ``full_docs.content`` value.

    The ``{{LRdoc}}`` marker is stripped **only** when ``parse_format``
    indicates the record is in madrag format.  Any other ``parse_format``
...
# ---------------------------------------------------------------------------
# madRAG Document blocks loader
# ---------------------------------------------------------------------------


async def load_madrag_document_content(sidecar_uri: str) -> tuple[str, str]:
    """Load madRAG Document blocks and return ``(merged_text, blocks_path)``.
    ...
    if resolved is None:
        raise FileNotFoundError(
            f"madRAG blocks file not found from sidecar uri: {sidecar_uri}"
        )
...
# :func:`madrag.operate.extract_entities._process_single_content` so each

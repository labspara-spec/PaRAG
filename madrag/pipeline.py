"""Document ingestion pipeline mixin for the madRAG class.

This module isolates the document parse/enqueue/extraction pipeline so that
``madrag.py`` stays focused on storage management, querying, and editing.
The mixin is wired into :class:`madrag.madRAG` via multiple inheritance
...
from madrag.operate import merge_nodes_and_edges
...
from madrag.utils_pipeline import (
    ...
    input_dir_path,
    load_madrag_document_content,
    make_madrag_doc_content,
    normalize_document_file_path,
    parsed_artifact_dir_for,
    resolve_doc_file_path,
    sidecar_blocks_path,
    sidecar_uri_for,
    strip_madrag_doc_prefix,
)
...
class _PipelineMixin:
    """Mixin providing document ingestion pipeline methods for madRAG.

    Designed to be combined as a base of madRAG only.  Relies on
    madRAG-provided attributes (``self.full_docs``, ``self.doc_status``,
    ...
    async def apipeline_enqueue_documents(
        ...
        madrag_document_paths: str | list[str] | None = None,
        ...
        1. Validate ids if provided or generate MD5 hash IDs and remove duplicate contents (skip content dedup when format is madrag)
        2. Generate document initial status
        3. Filter out already processed documents
        4. Enqueue document in status

        Args:
            input: Single document string or list of document strings (can be empty when docs_format is madrag)
            ids: list of unique document IDs, if not provided, MD5 hash IDs will be generated (from content or file_path when madrag)
            file_paths: list of file paths corresponding to each document, used for citation
            track_id: tracking ID for monitoring processing status
            docs_format: "raw" (default) or "madrag"; when "madrag" content may be empty and content-dedup is skipped
            madrag_document_paths: paths to madRAG Document (e.g. .blocks.jsonl dir or base path), when docs_format is madrag
            ...
            chunk_options: per-document chunker parameter snapshot.
                ...
                :func:`madrag.parser.routing.resolve_chunk_options`
                ...
                :meth:`madRAG.ainsert` — should call
        ...
        if isinstance(madrag_document_paths, str):
            madrag_document_paths = (
                [madrag_document_paths] if madrag_document_paths else None
            )
        ...
        is_madrag_format = docs_format == FULL_DOCS_FORMAT_LIGHTRAG
        if is_madrag_format and madrag_document_paths is not None:
            if len(madrag_document_paths) != len(input):
                raise ValueError(
                    "Number of madrag_document_paths must match the number of documents"
                )
        ...
        def _process_options_at(index: int) -> str:
            if process_options is None:
                return ""
            from madrag.parser.routing import sanitize_process_options
        ...
        def _chunk_options_at(index: int) -> dict[str, Any]:
            ...
            F-strategy runtime args (``split_by_character`` /
            ``split_by_character_only`` from :meth:`madRAG.ainsert`)
            are baked into the snapshot upstream — ainsert calls
            :func:`madrag.parser.routing.resolve_chunk_options` itself
        ...
        # 1. Validate ids and build contents (when madrag: no content dedup, content may be empty)
        if ids is not None:
        ...
        # Per-doc I/O failures from the madrag-format branch.  Populated when
        # ``load_madrag_document_content`` cannot read the user-supplied
        ...
        madrag_load_errors: list[dict[str, Any]] = []
        ...
        def _add_content(
            ...
            *,
            ...
            # strip_madrag_doc_prefix is a no-op for non-madrag formats.
            body_length = len(strip_madrag_doc_prefix(content, doc_format))
            ...
            if doc_format in (FULL_DOCS_FORMAT_RAW, FULL_DOCS_FORMAT_LIGHTRAG):
                content_hash = compute_text_content_hash(
                    strip_madrag_doc_prefix(content or "", doc_format)
                )
        ...
        if is_madrag_format:
            # madRAG Document: no content hash dedup; content may be empty
            for i in range(len(file_paths)):
                path = file_paths[i]
                raw_path = (
                    madrag_document_paths[i] if madrag_document_paths else ""
                ) or path
                ...
                # for format=madrag must be "{{LRdoc}}" + the merged body.
                ...
                try:
                    merged_text, _ = await load_madrag_document_content(
                        sidecar_location
                    )
                except Exception as exc:
                    error_msg = f"load_madrag_document_content failed: {exc}"
                    ...
                    if blocks_path_str:
                        try:
                            ...
                            file_size = 0
                    madrag_load_errors.append(
                        {
                            "file_path": path,
                            "error_description": (
                                "Failed to load madRAG Document blocks"
                            ),
                    ...
                    continue
                summary_content = make_madrag_doc_content(merged_text)
                _add_content(
                    i,
                    summary_content,
                    FULL_DOCS_FORMAT_LIGHTRAG,
        ...
        def _initial_doc_status(content_data: dict[str, Any]) -> dict[str, Any]:
            # For madrag-format full_docs the persisted content carries the
            # ``{{LRdoc}}`` marker; strip it so summary/length match raw
            # semantics (the marker is full_docs internal bookkeeping and
            # must not leak into doc_status).  strip_madrag_doc_prefix
            # internally checks parse_format, so non-madrag formats pass
            # through untouched.
            body_text = strip_madrag_doc_prefix(
        ...
        async with enqueue_serialize_lock:
            ...
            # Flush madrag-format I/O failures as FAILED stubs.  Done
            ...
            if madrag_load_errors:
                await self.apipeline_enqueue_error_documents(
                    madrag_load_errors, track_id=track_id
                )
            ...
            if not new_docs:
                ...
                # If FAILED stubs were just flushed (madrag-format I/O
    ...
    async def _analyze_worker(self, ctx: _BatchRunContext) -> None:
        ...
        body (pending_parse → madrag / raw documents start with empty
    ...
    async def process_single_document(
        ...
        *,
        ...
        """
        from madrag.parser.routing import parse_process_options
        ...
        async with ctx.semaphore:
            try:
                ...
                if not isinstance(chunk_opts, dict) or not chunk_opts:
                    ...
                    from madrag.parser.routing import resolve_chunk_options
                ...
                # parse_format=raw, which silently mislabelled madrag docs;
    ...
    async def parse_native(
        ...
        """Phase 1 parse for native/raw, madrag and pending_parse formats."""
        ...
        if doc_format == FULL_DOCS_FORMAT_LIGHTRAG:
            ...
            merged_text = strip_madrag_doc_prefix(
        ...
        if doc_format == FULL_DOCS_FORMAT_PENDING_PARSE:
            ...
            from madrag.parser.docx.drawing_image_extractor import (
            ...
            from madrag.sidecar import write_sidecar
            ...
            await self._persist_parsed_full_docs(
                doc_id,
                {
                    "content": make_madrag_doc_content(parsed_data["content"]),
                    "file_path": file_path,
    ...
    async def parse_mineru(
        ...
        :func:`madrag.api.routers.document_routes.delete_file_variants_by_file_path`.
        """
    ...
    async def parse_docling(
        ...
        from madrag.parser.external.docling import (
            DoclingIRBuilder,
            DoclingRawClient,
            clear_dir_contents,
            is_bundle_valid,
            ...
            if source_name:
                for candidate in matches:
                    if candidate.name == source_name:
                        return str(candidate)
            if parser_engine:
                from madrag.parser.routing import filename_parser_directives
    ...
    async def _write_madrag_document_from_content_list(
        self,
        doc_id: str,
        file_path: str,
        ...
        """Convert parser content list to madRAG Document files and return parsed_data."""
        document_name = normalize_document_file_path(file_path)
        if document_name == "unknown_source":
        ...
        # Keep full_docs in sync so restart/reprocess can directly use madRAG Document.
        await self._persist_parsed_full_docs(
            doc_id,
            {
                "content": make_madrag_doc_content(merged_text),
                "file_path": file_path,
                "parse_format": FULL_DOCS_FORMAT_LIGHTRAG,
                "sidecar_location": sidecar_uri_for(parsed_dir),
                "parse_engine": engine,
    ...
    async def analyze_multimodal(
        ...
        *,
        ...
        """Phase 2: Multimodal analysis (VLM). Writes llm_analyze_result to madRAG Document.
        ...
        # Docling, and pre-existing madRAG documents reused from disk
        ...
        if enabled_modalities and tokenizer is not None:
            try:
                from madrag.multimodal_context import (
        ...
        try:
            ...
            from madrag.llm._vision_utils import (
                image_audit_metadata,
                image_cache_metadata,
                normalize_image_inputs,
                read_image_dimensions,
            )
            from madrag.prompt_multimodal import (
                IMAGE_TYPE_ENUM,
                IMAGE_TYPE_FALLBACK,
                MULTIMODAL_PROMPTS,
            )
            ...
            # in madrag.utils).  When the flag is off we must NOT save the
            ...
            def _json_extract(text: str) -> dict[str, Any]:
                ...
                Mirrors :func:`madrag.operate._process_json_extraction_result`
            ...
            async def _analyze_text_modality(
                ...
                if tokenizer is not None:
                    from madrag.constants import DEFAULT_MAX_EXTRACT_INPUT_TOKENS
    ...
    def _build_mm_chunks_from_sidecars(
        ...
        def _render(
            ...
            # ``madrag.operate._parse_mm_display_name`` (regex
    ...
    async def _embed_drawing_images(
        ...
        async def _flush_batch() -> None:
            if not batch_items:
                return
            try:
                from madrag.llm.gemini import gemini_multimodal_embed

...
madRAG can route different stages of work (entity extraction, keyword
...
class RoleSpec:
    ...
    Adding a new role anywhere in madRAG is a single-line edit: append a
...
ROLES: tuple[RoleSpec, ...] = (
    ...
    RoleSpec("intent", "INTENT", "intent LLM func"),
)
...
@dataclass
class RoleLLMConfig:
    """Per-role LLM override accepted at :class:`madRAG` init time.
...
class _RoleLLMMixin:
    """Mixin that owns the role LLM runtime on :class:`madRAG`.

    Mixed into madRAG only. Relies on attributes that the main class

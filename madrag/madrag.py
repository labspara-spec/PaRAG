...
from madrag.chunker import chunking_by_token_size
from madrag.operate import (
    extract_entities,
    kg_query,
    naive_query,
    rebuild_knowledge_from_chunks,
)
from madrag.utils_pipeline import normalize_document_file_path
...
from madrag.pipeline import _PipelineMixin
from madrag.chunk_schema import CHUNK_EXTENDED_META_FIELDS
from madrag.kg.factory import get_storage_class
from madrag.addon_params import (
    ObservableAddonParams,
    normalize_addon_params,
)
from madrag.llm_roles import (
    ROLE_NAMES,
    ROLES,
    RoleLLMConfig,
    RoleSpec,  # noqa: F401  # re-exported via madrag/__init__.py
    _optional_env_int,
    _RoleLLMMixin,
    _RoleLLMState,
)
from madrag.storage_migrations import _StorageMigrationMixin
...
@final
@dataclass
class madRAG(_RoleLLMMixin, _StorageMigrationMixin, _PipelineMixin):
    ...
    :func:`madrag.parser.routing.default_chunker_config`.  Per-doc
    ...
    **When this function is actually invoked.** The chunker dispatch in
    ...
    document's ``process_options``:
      ...
      - If ``process_options`` explicitly contains a chunking selector
        ...
        :mod:`madrag.chunker` (``chunking_by_fixed_token`` for ``F``,
    ...
    :attr:`madrag.parser.routing.ProcessOptions.chunking_explicit`.

    **Signature** — preserved unchanged from earlier madRAG releases
    ...
    Defaults to :func:`madrag.chunker.chunking_by_token_size`.
    """
    ...
    Obtain via ``get_cloud_provider()`` from ``madrag.storage.cloud``.
    """
    ...
    # TODO: Deprecated (madRAG will never initialize storage automatically on creation，and finalize should be call before destroying)
    auto_manage_storages_states: bool = field(default=False)
    """If True, madrag will automatically calls initialize_storages and finalize_storages at the appropriate times."""
    ...
    def _apply_chunk_size_overlay(self) -> None:
        ...
        2. Strategy-specific env (``CHUNK_F_OVERLAP_SIZE`` /
           ...
           :func:`madrag.parser.routing.default_chunker_config` *only*
        ...
        3. Legacy constructor field
           (``madRAG(chunk_token_size=…, chunk_overlap_token_size=…)``).
    ...
    def __post_init__(self, addon_params: dict[str, Any] | None):
        ...
        logger.debug(f"madRAG init with param:\n  {_print_config}\n")
        ...
        # while preventing side effects when the same EmbeddingFunc is reused across multiple madRAG instances
    ...
    async def initialize_storages(self):
        ...
        if self._storages_status == StoragesStatus.CREATED:
            ...
            if default_workspace is None:
                ...
                logger.info(
                    f"Creating madRAG instance with workspace='{self.workspace}' "
                    f"while default workspace is set to '{default_workspace}'"
                )

            # Auto-initialize pipeline_status for this workspace
            from madrag.kg.shared_storage import initialize_pipeline_status
    ...
    async def adelete_by_entity(self, entity_name: str) -> DeletionResult:
        ...
        Returns:
            DeletionResult: An object containing the outcome of the deletion process.
        """
        from madrag.utils_graph import adelete_by_entity
    ...
    async def adelete_by_relation(
        ...
        Returns:
            DeletionResult: An object containing the outcome of the deletion process.
        """
        from madrag.utils_graph import adelete_by_relation
    ...
    async def get_entity_info(
        ...
        from madrag.utils_graph import get_entity_info
    ...
    async def get_relation_info(
        ...
        from madrag.utils_graph import get_relation_info
    ...
    async def aedit_entity(
        ...
        """
        from madrag.utils_graph import aedit_entity
    ...
    async def aedit_relation(
        ...
        """
        from madrag.utils_graph import aedit_relation
    ...
    async def acreate_entity(
        ...
        """
        from madrag.utils_graph import acreate_entity
    ...
    async def acreate_relation(
        ...
        """
        from madrag.utils_graph import acreate_relation
    ...
    async def amerge_entities(
        ...
        """
        from madrag.utils_graph import amerge_entities
    ...
    async def aexport_data(
        ...
        """
        from madrag.utils import aexport_data as utils_aexport_data
...
# passed through madRAG(addon_params=...). InitVars are not stored as
...
madRAG.addon_params = property(  # type: ignore[attr-defined]
    madRAG._get_addon_params,
    madRAG._set_runtime_addon_params,
)

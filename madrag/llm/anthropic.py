...
from madrag.llm.costpilot import wrap_with_costpilot
...
async def anthropic_complete_if_cache(
    ...
    **kwargs: Any,
    ...
    """Call Anthropic Messages API with madRAG-compatible shims.
    ...
    default_headers = {
        "User-Agent": f"Mozilla/5.0 (Macintosh; Intel Mac OS X 10_15_8) madRAG/{__api_version__}",
    ...
    if image_inputs:
        from madrag.llm._vision_utils import normalize_image_inputs
...
# ``madrag.llm.voyageai.voyageai_embed``. Keep the old name importable for one
...
def anthropic_embed(*args, **kwargs):
    """Deprecated alias for :func:`madrag.llm.voyageai.voyageai_embed`.
    ...
    warnings.warn(
        "madrag.llm.anthropic.anthropic_embed is deprecated and will be "
        "removed in a future release. Import "
        "madrag.llm.voyageai.voyageai_embed instead.",
        DeprecationWarning,
        stacklevel=2,
    )

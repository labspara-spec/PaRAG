"""Shared debug madRAG stand-in for the parse_* entry points.

A minimal ``madRAG`` stand-in plus a deterministic ``datetime`` shim,
shared by the unified parser debug CLI (``madrag/parser/cli.py``),
...
def build_debug_rag():
    """Build a minimal madRAG stand-in that exposes what ``parse_*`` reads.

    The import of ``madRAG`` is intentionally function-local: deferring
    it avoids a circular import when this helper is loaded during package
    init (the parser CLI resolves ``madrag.parser.debug`` before
    ``madrag`` itself is fully bound).

    madRAG-side attributes the three ``parse_*`` methods read off ``self`` —
    every entry MUST be provided by this stand-in, or the debug CLI / golden
    tests / regen script will all break in sync:

    - **methods** (rebound from :class:`madRAG`):
    ...
    When any of the three ``madRAG.parse_*`` methods grows a new
    ...
    class _DebugRag:
        ...
        parse_native = madRAG.parse_native
        parse_mineru = madRAG.parse_mineru
        parse_docling = madRAG.parse_docling

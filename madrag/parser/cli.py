...
Drives ``madRAG.parse_<engine>`` against a single source file and writes
...
Invocation::
    ...
    python -m madrag.parser.cli path/to/sample.docx --engine native
    python -m madrag.parser.cli path/to/sample.pdf  --engine mineru
    python -m madrag.parser.cli path/to/sample.pdf  --engine docling --force-reparse
...
def _build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        prog="parse_sidecar",
        description=(
            "Run madRAG.parse_<engine> on a single file and emit sidecar "
...
async def _run(args: argparse.Namespace) -> int:
    ...
    from madrag.parser.debug import build_debug_rag
    from madrag.utils import compute_mdhash_id
    import madrag.pipeline as pipeline_mod
    import madrag.utils_pipeline as utils_pipeline_mod

...
from madrag.utils import logger, parse_optional_float
...
class ProcessOptions:
    ...
    def chunking_explicit(self) -> bool:
        ...
        legacy externally-supplied :attr:`madRAG.chunking_func`.
        """
...
def default_chunker_config() -> dict[str, Any]:
    ...
    *strategy-specific* env vars (``CHUNK_F_OVERLAP_SIZE``,
    ...
    :meth:`madRAG._apply_chunk_size_overlay` apply the legacy
    constructor field (``madRAG(chunk_overlap_token_size=…)``) and
...
def resolve_chunk_options(
    ...
    *,
    ...
    The F runtime args from ``madRAG.ainsert`` overlay the

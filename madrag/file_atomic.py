...
Why this lives at the package root rather than under ``madrag/kg/``:
``madrag.utils.write_json`` needs ``atomic_write`` to gain crash safety,
and several ``madrag/kg/*`` modules need both. Hosting the helpers under
``madrag/kg/`` would create a ``utils -> kg -> utils`` import cycle.

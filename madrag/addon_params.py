...
``addon_params`` is a free-form configuration dict on :class:`madRAG` that
...
- :class:`ObservableAddonParams` — a ``dict`` subclass that calls a callback
  whenever the contents change so the madRAG runtime can invalidate cached
  derived state.
...
from madrag.constants import DEFAULT_SUMMARY_LANGUAGE
...
def default_addon_params() -> dict[str, Any]:
    ...
    # module via ``madRAG`` construction paths.
    from madrag.parser.routing import default_chunker_config

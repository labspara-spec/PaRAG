"""
Centralized configuration constants for madRAG.

This module defines default values for configuration constants used across
different parts of the madRAG system. Centralizing these values ensures
...
# madRAG Document pipeline
FULL_DOCS_FORMAT_RAW = "raw"  # content in full_docs["content"]
FULL_DOCS_FORMAT_LIGHTRAG = "madrag"  # content in madRAG Document files
FULL_DOCS_FORMAT_PENDING_PARSE = (
    "pending_parse"  # file saved but not yet parsed; parse_native will read from disk
)
# Marker prefix for full_docs.content when format=madrag.
...
# preview without loading the full madRAG Document file.
...
# Instance pool defaults (madRAGPool — one instance per workspace)
DEFAULT_POOL_MAX_SIZE = 8        # max concurrent madRAG instances in pool
...
# Worker service defaults (madrag-worker process)
...
DEFAULT_LOG_FILENAME = "madrag.log"  # Default log filename

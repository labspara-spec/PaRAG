...
to :data:`madrag.constants.DEFAULT_R_SEPARATORS`); only when a single
...
:func:`madrag.chunk_schema.strip_internal_multimodal_markup_for_extraction`
...
``madrag.operate._process_single_content``) and ensures the
...
from madrag.utils import Tokenizer
...
def load_chunk_separators() -> list[str]:
    ...
    :data:`madrag.constants.DEFAULT_R_SEPARATORS` on missing / invalid
...
def trim_content_to_budget(
    ...
    *,
    ...
    :data:`madrag.constants.DEFAULT_MAX_EXTRACT_INPUT_TOKENS`.  Only ``content``

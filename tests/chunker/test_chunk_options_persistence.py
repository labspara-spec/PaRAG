...
1. **env-driven snapshot**: env vars (CHUNK_R_OVERLAP_SIZE,
   ...
   :func:`madrag.parser.routing.default_chunker_config`, then into
...
from madrag.constants import DEFAULT_R_SEPARATORS
...
def test_env_driven_snapshot_persisted_in_full_docs(tmp_path, monkeypatch):
    ...
    async def _run():
        ...
        try:
            # F slot — mirror what ``madRAG.ainsert`` does: build the
...
def test_constructor_chunk_size_overlays_addon_params(tmp_path, monkeypatch):
    """``madRAG(chunk_token_size=N, chunk_overlap_token_size=M)`` must
...
def test_strategy_env_wins_over_legacy_ctor_field(tmp_path, monkeypatch):
    ...
    ``madRAG(chunk_overlap_token_size=2)`` is the legacy
...
def test_p_strategy_defaults_to_dedicated_size_when_env_unset(tmp_path, monkeypatch):
    ...
    ``madRAG(chunk_token_size=…)``.  Paragraph-semantic merging needs
    ...
    from madrag.constants import DEFAULT_CHUNK_P_SIZE
...
def test_r_strategy_falls_back_to_global_chunk_size(tmp_path, monkeypatch):
    ...
    from the standard chain (here: ``madRAG(chunk_token_size=…)``)."""

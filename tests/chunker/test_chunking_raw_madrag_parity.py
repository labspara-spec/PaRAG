"""F-chunking parity between raw and madrag formats.

After the F-chunking unification, ``apipeline_process_enqueue_documents``
strips the ``{{LRdoc}}`` marker from madrag-format content and feeds the
...
* T1: identical input text produces identical chunking inputs whether it
  arrives as raw or as a madrag ``.blocks.jsonl``.
* T2: ``full_docs.content`` for madrag carries the *full* merged text
...
* T3: ``extraction_meta["parse_format"]`` (surfaced via
  ``doc_status.metadata``) is now ``"madrag"`` for madrag docs —
...
* T6: a ``pending_parse`` document that resolves to madrag at parse
...
* T7: a raw document whose body *literally* starts with ``{{LRdoc}}``
  ...
  unconditional ``strip_madrag_doc_prefix`` at the chunking boundary
...
def _new_rag(tmp_path: Path, **kwargs) -> madRAG:
...
def _attach_chunking_spy(rag: madRAG) -> dict:
...
def _write_madrag_blocks(blocks_path: Path, body_paragraphs: list[str]) -> None:
    """Write a minimal valid madRAG ``.blocks.jsonl`` with body paragraphs."""
    lines = [
        json.dumps(
            {
                "type": "meta",
...
# ---------------------------------------------------------------------------
# T1 — parity: raw vs madrag produce identical chunking input
# ---------------------------------------------------------------------------


@pytest.mark.offline
def test_chunking_input_parity_raw_vs_madrag(tmp_path, monkeypatch):
    """Same body text in raw and madrag formats must reach
    ...
    async def _run():
        ...
        _write_madrag_blocks(blocks_path, paragraphs)
        ...
        try:
            await rag_lr.apipeline_enqueue_documents(
                "",
                file_paths="parity.madrag",
                docs_format=FULL_DOCS_FORMAT_LIGHTRAG,
                madrag_document_paths="__parsed__/parity.blocks.jsonl",
                track_id="track-lr",
            )
        ...
        assert spy_lr["calls"] >= 1, "madrag doc never reached chunking_func"
        assert spy_lr["input"] == spy_raw["input"] == expected_merged, (
            "chunking_func received different inputs for raw vs madrag; "
    ...
    async def _run():
        ...
        try:
            await rag.apipeline_enqueue_documents(
                "",
                file_paths="big.madrag",
                docs_format=FULL_DOCS_FORMAT_LIGHTRAG,
                madrag_document_paths="__parsed__/big.blocks.jsonl",
                track_id="track-big",
            )

            doc_id = compute_mdhash_id("big.madrag", prefix="doc-")
...
@pytest.mark.offline
def test_extraction_meta_records_madrag_parse_format(tmp_path, monkeypatch):
    """Before the unification, a structured-parse fallback tagged
    ``extraction_meta.parse_format = raw`` for madrag docs, silently
    ...
    async def _run():
        ...
        try:
            await rag.apipeline_enqueue_documents(
                "",
                file_paths="tag.madrag",
                docs_format=FULL_DOCS_FORMAT_LIGHTRAG,
                madrag_document_paths="__parsed__/tag.blocks.jsonl",
                track_id="track-tag",
            )
            await rag.apipeline_process_enqueue_documents()

            doc_id = compute_mdhash_id("tag.madrag", prefix="doc-")
            status_doc = await rag.doc_status.get_by_id(doc_id)
            assert status_doc is not None
            metadata = (
...
class _ListHandler(logging.Handler):
    ...
    The ``madrag`` logger has ``propagate = False`` so pytest's caplog
...
def test_explicit_R_dispatches_to_recursive_character(tmp_path, monkeypatch):
    ...
    import madrag.chunker as chunker_pkg
    from madrag.chunker import chunking_by_recursive_character as real_r
    ...
    # The dispatcher does ``from madrag.chunker import …`` inside the
    ...
    async def _run():
        ...
        madrag_logger.addHandler(list_handler)
        try:
            await rag.apipeline_enqueue_documents(
            ...
            madrag_logger.removeHandler(list_handler)
            await rag.finalize_storages()
...
def test_explicit_V_dispatches_to_semantic_vector(tmp_path, monkeypatch):
    ...
    async def _run():
        ...
        ), "dispatcher must hand the madRAG embedding_func to the V chunker"
        assert legacy_spy["calls"] == 0, (
...
# ---------------------------------------------------------------------------
# T6 — pending_parse → madrag summary is populated after PROCESSED
# ---------------------------------------------------------------------------


@pytest.mark.offline
def test_pending_parse_madrag_summary_populated_after_processed(
    ...
    async def _run():
        ...
        monkeypatch.setattr(
            "madrag.parser.docx.parse_document.extract_docx_blocks",
            _stub_extract,
        )
...
def test_raw_text_starting_with_marker_chunked_verbatim(tmp_path):
    ...
    characters.  ``strip_madrag_doc_prefix`` is a madrag-only contract

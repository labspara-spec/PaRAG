...
from madrag.utils import EmbeddingFunc, Tokenizer, priority_limit_async_func_call
...
@pytest.fixture
def madrag_logger_propagating(monkeypatch):
    """Force the madrag logger to propagate so caplog can capture records."""
...
def _make_rag(tmp_path, **kwargs) -> madRAG:
    """Create a madRAG for role tests.
...
async def test_priority_queue_graceful_shutdown_timeout_falls_back_to_force(
    caplog, madrag_logger_propagating
):
    started = asyncio.Event()
...
def test_role_max_async_defaults_inherit_base(tmp_path, monkeypatch):
    ...
    # (e.g. madrag.kg.networkx_impl) are imported lazily during
    # madRAG() and re-run load_dotenv(override=False), which would
...
def test_role_llm_config_logs_once_on_init_with_metadata(
    tmp_path, caplog, madrag_logger_propagating
):
    with caplog.at_level("INFO", logger="madrag"):
        rag = _make_rag(
            tmp_path,
            role_llm_configs={
                "query": RoleLLMConfig(
...
async def test_aupdate_llm_role_config_drain_timeout_does_not_propagate(
    tmp_path, monkeypatch, caplog, madrag_logger_propagating
):
    ...
    monkeypatch.setattr(madRAG, "_shutdown_llm_wrapper", fast_shutdown)

"""End-to-end test: native docx → madRAG Document → stable cache key.
...
``full_docs`` is in LIGHTRAG format. ``_load_madrag_document_content``
...
from madrag.utils import Tokenizer, TokenizerInterface, compute_args_hash
...
@pytest.mark.offline
def test_native_madrag_path_produces_stable_merged_text(tmp_path, monkeypatch):
    ...
    async def _run():
        ...
        # madRAG-specific writing — that is what we want under test.
        stable_blocks = [
            _block(
        ...
        result1 = await madRAG.parse_native(
            rag,
            "doc-stable",
            str(source_path),
        ...
        result2 = await madRAG.parse_native(
            rag,
            "doc-stable",
            str(source_path),
            {"parse_format": FULL_DOCS_FORMAT_PENDING_PARSE, "content": ""},
        ...
        assert record["parse_format"] == "madrag"
...
@pytest.mark.offline
def test_native_madrag_path_writes_blocks_jsonl_and_skips_meta_on_load(
    tmp_path, monkeypatch
):
    """Sanity check: ``_load_madrag_document_content`` must skip the
...
@pytest.mark.offline
def test_native_madrag_path_leaves_unknown_table_caption_empty(tmp_path, monkeypatch):
    ...
    async def _run():
        ...
        from madrag.multimodal_context import enrich_sidecars_with_surrounding
...
@pytest.mark.offline
def test_native_madrag_path_writes_image_assets_to_blocks_assets_dir(
    tmp_path, monkeypatch
):

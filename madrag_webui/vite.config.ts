 ...
 * `madrag/api/madrag_server.py`). Doing it in dev too means the SPA
 ...
 */
function madragRuntimeConfigPlugin(env: Record<string, string>): Plugin {
  ...
  return {
    name: 'madrag-dev-runtime-config',
    apply: 'serve',
...
export default defineConfig(({ mode }) => {
  ...
  return {
    plugins: [react(), tailwindcss(), madragRuntimeConfigPlugin(env)],
    resolve: {
      alias: {
    ...
    base: './',
    build: {
      outDir: path.resolve(__dirname, '../madrag/api/webui'),
      emptyOutDir: true,
